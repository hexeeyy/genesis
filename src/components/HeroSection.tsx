...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    // Hero text animation with stagger and advanced easing
    ...
    // Multi-layer parallax for pattern overlay
    ...
    // Advanced text parallax with 3D transforms
    ...
    // Photo parallax with perspective
    ...
    gsap.set([textRef.current, photoRef.current], { perspective: 1000 });
  ...
  const handleViewWork = () => {
    ...
    if (projectsSection) {
      gsap.to(window, { duration: 1.5, scrollTo: projectsSection, ease: "power3.inOut" });
    }
  };
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden hero-bg botanical-decoration">
      ...
      {/* Background decorative elements with advanced parallax */}
      <div ref={bgElementsRef} className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/3 right-1/4 w-24 h-24 bg-sage/10 rounded-full blur-2xl"></div>
        <div className="absolute bottom-1/3 left-1/4 w-28 h-28 bg-ocean/10 rounded-full blur-2xl"></div>
      </div>
      
      {/* Victorian pattern overlay with advanced parallax */}
