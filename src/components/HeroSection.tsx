...
const HeroSection = () => {
  ...
  if (heroRef.current) {
      const animation = gsap.to(heroRef.current, {
        yPercent: -10, // Subtle parallax movement
        ease: 'none',
        scrollTrigger: {
          trigger: heroRef.current,
  ...
  return (
    <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden hero-bg botanical-decoration">
      ...
      >
        <div className="bg-gradient-to-r from-palette-sage to-palette-ocean backdrop-blur-sm border border-palette-victorian-cream p-3 rounded-full shadow-md hover:shadow-victorian transition-all duration-30 animate-bounce">
          <ChevronDown className="text-palette-victorian-cream w-5 h-5" aria-hidden="true" />
        </div>
      </div>

      {/* Background decorative elements with 1860s theme */}
      <div ref={bgElementsRef} className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-8 w-32 h-32 bg-palette-victorian-gold/20 rounded-full blur-3xl floating-element animate-subtle-breathe"></div>
        <div className="absolute bottom-1/4 right-8 w-40 h-40 bg-palette-victorian-dusty-blue/20 rounded-full blur-3xl floating-element animate-gentle-float"></div>
      </div>
    </section>
  );
};
